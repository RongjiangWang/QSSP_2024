...
c
      ...
      real*8 f,x,dt0,df0,rn,re,azi,bazi,bazi0,rr0,rrs
...
c
      do is=1,ns
        if(istf(is).eq.0)then
          call swavelet(trss(is),df,fi,nf,wvf(1,is))
        else
          call brunewvlet(trss(is),df,fi,nf,istf(is),wvf(1,is))
        endif
        if(.not.eqs(is))then
          do lf=1,nf
            wvf(lf,is)=wvf(lf,is)*dcmplx(-PI2*fi,PI2*dble(lf-1)*df)
          enddo
        endif
      enddo
c
...
c
        lys=lygrn(ig)
        rrs=rrup(lys)
        if(grnrr0(ig).gt.0.d0)then
          x=dcos(grnrr0(ig)/rrs)
          disk(0)=1.d0
          disk(1)=(3.d0+x)/4.d0
          do ldeg=2,ldegup
...
c
          if(grnrr0(ig).gt.0.d0)then
            do istp=1,6
              do ldeg=0,ldegf
                ul0(ldeg,istp)=ul0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                vl0(ldeg,istp)=vl0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                wl0(ldeg,istp)=wl0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                el0(ldeg,istp)=el0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                fl0(ldeg,istp)=fl0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                gl0(ldeg,istp)=gl0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                pl0(ldeg,istp)=pl0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
                ql0(ldeg,istp)=ql0(ldeg,istp)*dcmplx(disk(ldeg),0.d0)
              enddo
            enddo
          endif
c
